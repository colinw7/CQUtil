...
#ifdef CQSTYLE_TAB_BAR
void
tabLayout(const QStyle *proxyStyle, const QStyleOptionTab *opt, const QWidget *widget,
          QRect *textRect, QRect *iconRect)
{
  Q_ASSERT(textRect);
  Q_ASSERT(iconRect);
  QRect tr = opt->rect;
...
void
CQStyle::
drawControl(ControlElement element, const QStyleOption *opt, QPainter *p,
    ...
    case CE_TabBarTabLabel: {
      if (const auto *tab = qstyleoption_cast<const QStyleOptionTab *>(opt)) {
        ...
        if (!tab->icon.isNull()) {
          ...
          auto tabIcon = tab->icon.pixmap(qt_getWindow(widget), tab->iconSize,
                           (tab->state & State_Enabled) ? QIcon::Normal : QIcon::Disabled,
                           (tab->state & State_Selected) ? QIcon::On : QIcon::Off);
          p->drawPixmap(iconRect.x(), iconRect.y(), tabIcon);
        }

        proxy()->drawItemText(p, tr, alignment, tab->palette, tab->state & State_Enabled,
