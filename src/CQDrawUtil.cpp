...
void qDrawShadeLine(QPainter *p, int x1, int y1, int x2, int y2, const QPalette &pal, bool sunken,
                    int lineWidth, int midLineWidth)
{
  ...
  if (y1 == y2) {                                // horizontal line
    ...
    for (i=0; i<lineWidth; i++) {                // draw top shadow
      a.setPoints(3, x1+i, y+tlw-1-i, x1+i, y+i, x2-i, y+i);
    ...
    if (midLineWidth > 0) {
      ...
      for (i=0; i<midLineWidth; i++)        // draw lines in the middle
        p->drawLine(x1+lineWidth, y+lineWidth+i, x2-lineWidth, y+lineWidth+i);
    }
    ...
    for (i=0; i<lineWidth; i++) {                // draw left shadow
      a.setPoints(3, x+i, y2, x+i, y1+i, x+tlw-1, y1+i);
    ...
    for (i=0; i<lineWidth; i++) {                // draw right shadow
      a.setPoints(3, x+lineWidth, y2-i, x+tlw-i-1, y2-i, x+tlw-i-1, y1+lineWidth);
...
/*!
  \fn void qDrawPlainRect(QPainter *painter, int x, int y, int width, int height,
                   const QColor &lineColor, int lineWidth, const QBrush *fill)
  \relates <qdrawutil.h>
...
void qDrawShadeRect(QPainter *p, const QRect &r, const QPalette &pal, bool sunken,
                    int lineWidth, int midLineWidth, const QBrush *fill)
{
...
void qDrawShadePanel(QPainter *p, const QRect &r, const QPalette &pal, bool sunken,
                     int lineWidth, const QBrush *fill)
{
...
static void qDrawMotifArrow(QPainter *p, Qt::ArrowType type, bool down,
                           int x, int y, int w, int h, const QPalette &pal, bool)
{
...
void qDrawItem(QPainter *p, Qt::GUIStyle gs, int x, int y, int w, int h, int flags,
  ...
  if (pixmap) {
    ...
    if (! enabled) {
      if (pm.hasAlphaChannel()) {                        // pixmap with a mask
        ...
        QString k = QLatin1Literal("$qt-drawitem") % HexString<qint64>(pm.cacheKey());
...
/*!
  \fn void qDrawBorderPixmap(QPainter *painter, const QRect &target,
                       ...
                       QDrawBorderPixmap::DrawingHints hints)
{
  ...
  // corners
  if (targetMargins.top() > 0 && targetMargins.left() > 0 &&
      sourceMargins.top() > 0 && sourceMargins.left() > 0) { // top left
    int index = bool(hints & QDrawBorderPixmap::OpaqueTopLeft);
    sourceData[index].append(QRectF(sourceRect.topLeft(),
      QSizeF(sourceMargins.left(), sourceMargins.top())));
    targetData[index].append(QRectF(QPointF(xTarget[0], yTarget[0]),
                                    QPointF(xTarget[1], yTarget[1])));
  }
  if (targetMargins.top() > 0 && targetMargins.right() > 0 &&
      sourceMargins.top() > 0 && sourceMargins.right() > 0) { // top right
    int index = bool(hints & QDrawBorderPixmap::OpaqueTopRight);
    sourceData[index].append(QRectF(QPointF(sourceCenterRight, sourceRect.top()),
      QSizeF(sourceMargins.right(), sourceMargins.top())));
    targetData[index].append(QRectF(QPointF(xTarget[columns-1], yTarget[0]),
                                    QPointF(xTarget[columns], yTarget[1])));
  }
  if (targetMargins.bottom() > 0 && targetMargins.left() > 0 &&
      sourceMargins.bottom() > 0 && sourceMargins.left() > 0) { // bottom left
    int index = bool(hints & QDrawBorderPixmap::OpaqueBottomLeft);
    sourceData[index].append(QRectF(QPointF(sourceRect.left(), sourceCenterBottom),
      QSizeF(sourceMargins.left(), sourceMargins.bottom())));
    targetData[index].append(QRectF(QPointF(xTarget[0], yTarget[rows - 1]),
                                    QPointF(xTarget[1], yTarget[rows])));
  }
  if (targetMargins.bottom() > 0 && targetMargins.right() > 0 &&
      sourceMargins.bottom() > 0 && sourceMargins.right() > 0) { // bottom right
    int index = bool(hints & QDrawBorderPixmap::OpaqueBottomRight);
    sourceData[index].append(QRectF(QPointF(sourceCenterRight, sourceCenterBottom),
      QSizeF(sourceMargins.right(), sourceMargins.bottom())));
    targetData[index].append(QRectF(QPointF(xTarget[columns - 1], yTarget[rows - 1]),
                                    QPointF(xTarget[columns], yTarget[rows])));
  }
  ...
  if (targetCenterWidth > 0 && sourceCenterWidth > 0) {
    if (targetMargins.top() > 0 && sourceMargins.top() > 0) { // top
      ...
      for (int i = 1; i < columns - 1; ++i) {
        if (rules.horizontal == Qt::RepeatTile && i == columns - 2) {
          sourceData[index].append(QRectF(QPointF(sourceCenterLeft, sourceRect.top()),
            QSizeF(xTarget[i + 1] - xTarget[i], sourceMargins.top())));
        } else {
          sourceData[index].append(QRectF(QPointF(sourceCenterLeft, sourceRect.top()),
            QSizeF(sourceCenterWidth, sourceMargins.top())));
        }
        targetData[index].append(QRectF(QPointF(xTarget[i], yTarget[0]),
                                        QPointF(xTarget[i + 1], yTarget[1])));
      }
    }
    if (targetMargins.bottom() > 0 && sourceMargins.bottom() > 0) { // bottom
      ...
      for (int i = 1; i < columns - 1; ++i) {
        if (rules.horizontal == Qt::RepeatTile && i == columns - 2) {
          sourceData[index].append(QRectF(QPointF(sourceCenterLeft, sourceCenterBottom),
            QSizeF(xTarget[i + 1] - xTarget[i], sourceMargins.bottom())));
        } else {
          sourceData[index].append(QRectF(QPointF(sourceCenterLeft, sourceCenterBottom),
            QSizeF(sourceCenterWidth, sourceMargins.bottom())));
        }
        targetData[index].append(QRectF(QPointF(xTarget[i], yTarget[rows - 1]),
                                        QPointF(xTarget[i + 1], yTarget[rows])));
      }
    }
  }
  ...
  if (targetCenterHeight > 0 && sourceCenterHeight > 0) {
    if (targetMargins.left() > 0 && sourceMargins.left() > 0) { // left
      ...
      for (int i = 1; i < rows - 1; ++i) {
        if (rules.vertical == Qt::RepeatTile && i == rows - 2) {
          sourceData[index].append(QRectF(QPointF(sourceRect.left(), sourceCenterTop),
            QSizeF(sourceMargins.left(), yTarget[i + 1] - yTarget[i])));
        } else {
          sourceData[index].append(QRectF(QPointF(sourceRect.left(), sourceCenterTop),
            QSizeF(sourceMargins.left(), sourceCenterHeight)));
        }
        targetData[index].append(QRectF(QPointF(xTarget[0], yTarget[i]),
                                        QPointF(xTarget[1], yTarget[i + 1])));
      }
    }
    if (targetMargins.right() > 0 && sourceMargins.right() > 0) { // right
      ...
      for (int i = 1; i < rows - 1; ++i) {
        if (rules.vertical == Qt::RepeatTile && i == rows - 2) {
          sourceData[index].append(QRectF(QPointF(sourceCenterRight, sourceCenterTop),
            QSizeF(sourceMargins.right(), yTarget[i + 1] - yTarget[i])));
        } else {
          sourceData[index].append(QRectF(QPointF(sourceCenterRight, sourceCenterTop),
            QSizeF(sourceMargins.right(), sourceCenterHeight)));
        }
        targetData[index].append(QRectF(QPointF(xTarget[columns - 1], yTarget[i]),
                                        QPointF(xTarget[columns], yTarget[i + 1])));
      }
    }
  }
  ...
  if (targetCenterWidth > 0 && targetCenterHeight > 0 &&
      sourceCenterWidth > 0 && sourceCenterHeight > 0) {
    int index = bool(hints & QDrawBorderPixmap::OpaqueCenter);
    for (int j = 1; j < rows - 1; ++j) {
      qreal sourceHeight = (rules.vertical == Qt::RepeatTile && j == rows - 2) ?
        yTarget[j + 1] - yTarget[j] : sourceCenterHeight;
      for (int i = 1; i < columns - 1; ++i) {
        qreal sourceWidth = (rules.horizontal == Qt::RepeatTile && i == columns - 2) ?
          xTarget[i + 1] - xTarget[i] : sourceCenterWidth;
        sourceData[index].append(QRectF(QPointF(sourceCenterLeft, sourceCenterTop),
          QSizeF(sourceWidth, sourceHeight)));
        targetData[index].append(QRectF(QPointF(xTarget[i], yTarget[j]),
                                        QPointF(xTarget[i + 1], yTarget[j + 1])));
      }
    }
  }
  ...
  for (int i = 0; i < 2; ++i) {
    if (sourceData[i].size())
      painter->drawPixmapFragments(targetData[i].data(), sourceData[i].data(),
                                   sourceData[i].size(), pixmap,
